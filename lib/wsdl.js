...
BindingElement.prototype.postProcess = function(definitions, wsdlOptions) {
  ...
  } else if (wsdlOptions.useBindingDefinitions && typeof children !== 'undefined') {
    for (var i = 0, child; child = children[i]; i++) {
      if (child.name !== 'operation')
        continue;
      child.postProcess(definitions, 'binding');
      child.input.$name = child.$name;
      child.output.$name = child.$name;
      child.input.targetNSAlias = this.$type.substr(0, this.$type.indexOf(':'));
      this.methods[child.$name] = child;
      children.splice(i--, 1);
    }
  }
...
ServiceElement.prototype.postProcess = function(definitions, wsdlOptions) {
  var children = this.children,
  ...
  for (var i = 0, child; child = children[i]; i++) {
    ...
    if (binding) {
      binding.postProcess(definitions, wsdlOptions);
    ...
    self.processIncludes(function(err) {
      ...
      if (services) {
        for (name in services) {
          services[name].postProcess(self.definitions, self.options);
        }
      }
...
WSDL.prototype._initializeOptions = function (options) {
  ...
  this.options.useBindingDefinitions = options.useBindingDefinitions? true : false;
};
  ...
  p.onopentag = function(node) {
    ...
    if (!objectName && top.name === 'Body' && name !== 'Fault') {
      ...
      if (!message) {
        ...
        if (isInput)
          name = portType? portType.methods[name].input.$name : null;
        else
          name = portType? portType.methods[name].output.$name : null;
        message = self.definitions.messages[name];
        // 'cache' this alias to speed future lookups
        self.definitions.messages[originalName] = self.definitions.messages[name];
      }

      topSchema = message? message.description(self.definitions) : null;
